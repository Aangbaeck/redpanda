...
def skip_file_in_cdt(
    ...
    and nothing is marked. No-op locally and in dockerized CI.
    ...
    marked too, so its subclasses are covered via the shared function object. A
    subclass here whose tests are inherited from ANOTHER module is not covered
    (those tests belong to that module; skip them there or with a method-scope
    @skip_in_cdt). Unlike the class decorator this does not raise, since
    module-scope means "the tests defined in this file". `namespace` defaults to
    the caller's module globals; pass an explicit dict in unit tests.
    """
